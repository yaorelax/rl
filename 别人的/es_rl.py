...
def get_reward(network_param, num_p, env, ep_max_step, continuous_a, seed_and_id=None, ):
    ...
    if seed_and_id is not None:
        ...
        for layer in Net.children():
            layer.weight.data += params[:layer.weight.shape[0] * layer.weight.shape[1]].view(layer.weight.shape[0],
                                                                                             layer.weight.shape[1])
            layer.bias.data += params[
                               layer.weight.shape[0] * layer.weight.shape[1]:layer.bias.shape[0] + layer.weight.shape[
                                   0] * layer.weight.shape[1]]
...
def get_action(network, x, continuous_a):
    ...
    if not continuous_a[0]:
        return np.argmax(x.detach().numpy(), axis=1)[0]  # for discrete action
    else:
        return continuous_a[1] * np.tanh(x.detach().numpy())[0]  # for continuous action
...
def train(network_param, num_p, optimizer, utility, pool):
    ...
    for layer in network_param.keys():
        if 'weight' in layer:
            network_param[layer] += gradients[:network_param[layer].shape[0] * network_param[layer].shape[1]].view(
                network_param[layer].shape[0], network_param[layer].shape[1])
